...
// Debugged "last" error -- DMC
// last-modified: Thu Sep 24 17:14:31 2015
//
...
void init()
{
  ...
  if (rquat_loc == -1) {
    std::cerr << "Unable to find rquat in shader" << std::endl;
  }
...
extern "C" void display()
{
  ...
  if (trackballMove) {
    rquat = multq(vec4(cos(angle/2.0), sin(angle/2.0)*axis[0], 
		       sin(angle/2.0)*axis[1], sin(angle/2.0)*axis[2]), rquat);
  }
  // Send the new quaternion.
...
extern "C" void mouseButton(int button, int state, int x, int y)
{
  // use right button to start mouse tracking when down and stop when up
  // DMC -- This should be a switch(button), not nested ifs.
