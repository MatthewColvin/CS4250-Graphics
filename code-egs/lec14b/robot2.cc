// Robot program 
// nitially from Angel's book code.

// This version is uses simple boxes for all parts of the robot.
// This one is a bit drunken (see mods)

// Shows use of instance transformation to define parts (symbols) 

// Modified by David Chelberg
// last-modified: Sun Oct 29 20:04:11 2017
// Fixed a couple of bugs (e.g. GL_DEPTH_TEST vs. GL_DEPTH)
// 
#include "Angel.h"

// Number of miliseconds to delay for timer function
...
// Note use of push/pop to return modelview matrix to its state before
// functions were entered and use rotation, translation, and scaling to
// create instances of symbols (cube).
void base()
{
...
void timer(int val)
{
  ...
  Delta[0]=Theta[0]+f*2+30.0*sin(.2*f);
  Delta[1]=Theta[1]+f*3+20.0*cos(f);
  Delta[2]=Theta[2]+f*5+50.0*sin(f);
  
  /*  Delta[0]=Theta[0]+f*2+10.0*sin(f);
  Delta[1]=Theta[1]+f*2+10.0*sin(f);
  Delta[2]=Theta[2]+f*2+10.0*sin(f);
  */
  f+=0.1;
  glutPostRedisplay();
...
int main( int argc, char **argv )
{
  ...
  glutCreateWindow( "drunken robot" );
