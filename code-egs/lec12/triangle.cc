...
// Uses old text drawing routines (not new OpenGL compliant)
// last-modified: Thu Sep 25 19:22:30 2014
...
// Auxillary function to draw text at a position on screen.
...
extern "C" void mouse(int btn, int state, int x, int y)
{
  ...
  if (MyState == 1) {
    if(btn==GLUT_LEFT_BUTTON && state==GLUT_DOWN) {
      ...
      tri_y = wh -y;
      glClear(GL_COLOR_BUFFER_BIT);
      char *message=(char *)"Draw one vector for the triangle";
  ...
  // that will define the triangle we are drawing.
    ...
    if(btn==GLUT_LEFT_BUTTON && state==GLUT_DOWN) {
      ...
      triv1_y=(wh-y)-inity;
      glClear(GL_COLOR_BUFFER_BIT);
      char *message=(char *)"Draw second vector for the triangle";
      drawText(100,100,message);
      drawSquare(tri_x, tri_y);
      glFlush();
      ++MyState;
  ...
  // MyState==5 means we want to define a new triangle.
    ...
    if (btn==GLUT_LEFT_BUTTON && state==GLUT_UP) {
      glClear(GL_COLOR_BUFFER_BIT);
      char *message=(char *)"Please click on origin for new triangle";
      drawText(100,100,message);
      glFlush();
      MyState=1;
    }
...
extern "C" void display(void)
{
  ...
  case 0:
    ...
    {
      char *message=(char *)"Please click on origin for triangle";
      glColor3f(0.0, 1.0, 0.0);// Use green.
      drawText(100,100,message);
      glFlush();
    }
    ++MyState;
